...
load("C:/Users/yxt06/Desktop/panama-paper/complete data3.RData")
...
Entity50 = entity[(entity$countries %in% Top50_countries),]
Edges50 = edges[(edges$countries %in% Top50_countries),]
...
for (i in 1:50) {
  subset_country = Edges50[(Edges50$countries == Top50_countries[i]),]
  Top50_list[[i]] = subset_country
}

#2000 till 2010
for (i in 2000:2000){
  for (c in 1: 1) {
    ind_year = (Top50_list[[c]]$linkStart <= i) & (Top50_list[[c]]$linkEnd >= i)
    subset_year = Top50_list[[c]][ind_year,]
    ...
    g = add.vertices(g,length(unique(subset_year$index_1)), color = "orangered1", type=rep(TRUE,length(unique(subset_year$index_1))))
    ...
    gg = add.edges(g, edges_add, color = "orangered1")
    ...
    png(filename=filename, 
        ...
        res=72)
    cex.main = 1
    plot(gg,main = title, vertex.size = 2,  margin = 0.01, vertex.label=NA)
    dev.off()
